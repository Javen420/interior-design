...
function pickItems(
  ...
  let filtered = catalog.filter(
    (item) =>
      ...
      (item.colorTones.includes(colorTone) || item.colorTones.includes('neutral'))
  );
  ...
  const needed = roomSets[roomType] || roomSets['living-room'];
  ...
  for (const [category, count] of Object.entries(needed)) {
    ...
    // Sort by price
    categoryItems.sort((a, b) => (isBudgetFriendly ? a.price - b.price : b.price - a.price));
  ...
  while (totalCost > budgetMax && selected.length > 3) {
    const mostExpensive = selected.reduce((prev, curr) => (curr.price > prev.price ? curr : prev));
    const idx = selected.indexOf(mostExpensive);
    const cheaper = catalog
      .filter((i) => i.category === mostExpensive.category && i.price < mostExpensive.price)
...
function placeItems(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (['Seating'].includes(item.category)) {
      ...
      rotation = 0;
    } else if (item.category === 'Bedroom' && item.name.toLowerCase().includes('bed')) {
      ...
      if (bedItem) {
        const sideTableCount = placed.filter(p => p.name.toLowerCase().includes('nightstand') || p.name.toLowerCase().includes('side table')).length;
        if (sideTableCount === 0) {
...
export async function POST(request: NextRequest) {
  ...
  const template = (roomTemplates as Array<{
    id: string; flatType: string; roomType: string; width: number; length: number; sqm: number;
    averageCost: number;
    walls: { x1: number; y1: number; x2: number; y2: number }[];
    doors: { x: number; y: number; width: number; side: string }[];
    windows: { x: number; y: number; width: number; side: string }[];
  }>).find(
    (t) => t.flatType === body.flatType && t.roomType === body.roomType
  );
  if (!template) {
  ...
  const styleProfile = (styleProfiles as Array<{
    id: string; name: string; colorPalette: string[];
  }>).find(
    (p) => p.name.toLowerCase() === (body.styles[0] || '').toLowerCase()
  );
